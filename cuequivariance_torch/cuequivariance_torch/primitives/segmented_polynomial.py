...
class SegmentedPolynomial(nn.Module):
    ...
    def forward(
        ...
        #print(f"========= cuet op_name:{self.op_name} ==========")
        if False:
            #print(f"desc:{self.descriptor},  self.num_segments_list:{ self.num_segments_list}")
            ...
            y = inputs[2].view(-1, y_seg_num, self.u_dim)
            ...
            for pid, psi in  enumerate(self.path_segment_indices):
                ...
                w_seg = w[0, i, :] # # w_seg shape: [1694, U]
                x_seg = x_src[:, j, :] # x_seg shape: [1694, U]
                y_seg = y_src[:, k, :] # y_seg shape: [1694, 1]
                #print(f"w_seg shape:{w_seg.shape}, x_seg shape:{x_seg.shape}, y_seg shape:{y_seg.shape}")
